...
function harness() {
  ...
  let analytics = true;
  window.licanConsent = {
    getSnapshot: () => ({ analytics }),
    trackPageView(pagePath) {
      if (!analytics || typeof window.gtag !== 'function') return false;
  ...
  return {
    ...
    consent(value) { analytics = value; vm.runInContext('AnalyticsTracker()', context); },
    navigate(path) {
...
test('consent blocks events, accepts current page once and revokes immediately', () => {
  const h = harness();
  h.navigate('/'); h.consent(false); h.flush();
  assert.equal(h.calls.length, 0);
  h.navigate('/eventos'); h.flush();
  assert.equal(h.calls.length, 0);
  h.consent(true); h.strictReplay(); h.flush();
  assert.equal(h.calls.length, 1);
  h.consent(false); h.consent(true); h.flush();
  assert.equal(h.calls.length, 1);
  h.navigate('/contacto'); h.consent(false); h.flush();
  assert.equal(h.calls.length, 1);
});

test('all entrypoints use consent bootstrap and no unconditional Google tag', () => {
  for (const file of ['index.html', 'public/descubre-tu-bass/index.html', 'public/headbangdealers_the_game/index.html']) {
    const html = read(file);
    assert.equal((html.match(/src="\/consent.js"/g) || []).length, 1);
